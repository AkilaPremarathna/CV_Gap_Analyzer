...
Processes CVs uploaded by the user, stores data in MongoDB, ranks based on job role.
...
def extract_cv_text(cv_path):
    ...
    try:
        ...
        return f"Error: Reading Failed - {str(e)[:50]}"

# Cell 6: Gemini Data Extraction
...
# Cell 9: Get Skill Weights from Gemini
@st.cache_data(ttl=1800)
def get_skill_weights_for_job_role(_job_role, _required_skills):
    """
...
# Cell 10: Function to process a single CV
def process_single_cv(cv_path, job_role, batch_id, db_collection_handle):
    ...
    result = {"filename": cv_filename, "status": "failed", "error": None}
...
# Cell 11: Batch CV Processing Function
...
def rank_processed_cvs(batch_id, job_role, extracted_data_collection, job_role_collection):
    ...
    try:
        ...
        if not processed_cv_data_list:
            ...
            st.session_state.current_batch_cv_data = []
            return []
    ...
    with st.spinner(f"Calculating scores and ranking {len(processed_cv_data_list)} CVs..."):
        ...
        for cv_data in processed_cv_data_list:
            ...
            if not isinstance(extracted_skills_raw, list):
...
def get_chatbot_response(user_question, cv_context_summary, chat_history):
    ...
    model_history = []
    if chat_history:
        for msg in chat_history[-(history_limit*2):]:
...
uploaded_files = st.sidebar.file_uploader(
    "Upload CV Files (PDFs):",
    type=["pdf"],
    accept_multiple_files=True,
    help="Upload the PDF CV files you want to process."
)

# Save uploaded files to a temporary directory
if uploaded_files:
    temp_dir = tempfile.mkdtemp()
    cv_folder_path = temp_dir

    for uploaded_file in uploaded_files:
        file_path = os.path.join(temp_dir, uploaded_file.name)
        with open(file_path, "wb") as f:
            f.write(uploaded_file.getbuffer())
    st.sidebar.caption(f"Uploaded {len(uploaded_files)} CV files to temporary folder: `{temp_dir}`")
else:
    cv_folder_path = None
...
if st.sidebar.button("▶️ Run Processing & Ranking", disabled=run_disabled, use_container_width=True):
    ...
    # --- Start Processing ---
    st.info(f"Starting processing for job role: **{selected_job_role}** with {len(uploaded_files)} uploaded CVs")
    ...
    if cv_folder_path and os.path.isdir(cv_folder_path):
        shutil.rmtree(cv_folder_path, ignore_errors=True)

# --- Display Area ---
st.header("Results")

if not st.session_state.processing_done and not st.session_state.ranking_done:
    st.info("Upload CV files and select a job role in the sidebar, then click 'Run Processing & Ranking'.")
